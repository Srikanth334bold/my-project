...
#flash {
    ...
    transition: all 0.06s linear;   /* ULTRA FAST */
}
...
/* Load voices once (important!) */
function loadVoices() {
    const voices = window.speechSynthesis.getVoices();

    // Prefer Indian / English natural voices if available
    preferredVoice =
...
function speakTray(packetType, color) {
    if (!("speechSynthesis" in window)) return;

    const msg = new SpeechSynthesisUtterance();

    // Natural phrasing
    msg.text = `${packetType} ${color}`;
...
evtSource.onmessage = function(event) {
    ...
    if (data.event && data.event_id !== lastEventId) {
        ...
        /* 🔊 VOICE ANNOUNCEMENT (packet + color) */
        speakTray(data.event.packet_type, data.event.color);
