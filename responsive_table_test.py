...
evtSource.onmessage = function(event) {
    ...
    if (data.event && data.event_id !== lastEventId) {
        ...
        else if (pkt === "homegenised") flash.classList.add("flash-homegenised");
...
// ADJUST FONT SIZE TO FIT ALL ROWS
function adjustFontSize() {
    ...
    const flashHeight = document.getElementById("flash").offsetHeight || 0;
    const padding = 40;
    const availableHeight = window.innerHeight - headerHeight - flashHeight - padding;

    const fontSize = Math.floor(availableHeight / rows.length / 1.8);
