...
def create_audio(packet_type, color):
    ...
    try:
        ...
        # Try to find Indian voices in order of preference
        indian_voice_found = False
        selected_voice = None
        
        # First try Hindi voice
        for voice in voices:
            if voice.id == 'inc/hi' or 'hindi' in voice.name.lower():
                engine.setProperty('voice', voice.id)
                selected_voice = voice
                indian_voice_found = True
                print(f'[INFO] Using Hindi voice: {voice.name}')
                break
        
        # If no Hindi, try other Indian English or regional voices
        if not indian_voice_found:
            for voice in voices:
                if ('inc/' in voice.id or 'dra/' in voice.id or 
                    'bengali' in voice.name.lower() or 'tamil' in voice.name.lower() or
                    'telugu' in voice.name.lower() or 'gujarati' in voice.name.lower() or
                    'marathi' in voice.name.lower() or 'kannada' in voice.name.lower()):
                    engine.setProperty('voice', voice.id)
                    selected_voice = voice
                    indian_voice_found = True
                    print(f'[INFO] Using Indian voice: {voice.name}')
                    break
        
        # If still no Indian voice found, use default English
        if not indian_voice_found:
            print('[INFO] No Indian voice found, using default English voice')
        ...
        if os.path.exists(file_path):
            ...
            if file_size > 0:
                print(f'[INFO] Audio generated: {file_name} ({file_size} bytes)')
            else:
                print(f'[WARN] Audio file created but empty: {file_name}')
                # Try alternative method using speech-dispatcher directly
                try:
                    import subprocess
                    # Use spd-say if available (speech-dispatcher)
                    result = subprocess.run(['spd-say', '-t', 'male1', '-r', '10', text], 
                                          capture_output=True, timeout=5)
                    if result.returncode == 0:
                        print('[INFO] Used speech-dispatcher as fallback')
                except Exception:
                    pass
        else:
            print(f'[WARN] Audio file not created: {file_name}')
            
    except Exception as e:
        print(f'[WARN] TTS generation failed: {e}')
        # Create a dummy file to prevent repeated attempts
        try:
...
/* Packet type color mapping (EXACT SPELLINGS) */
.flash-shubham     { background: #f97316; color: white; }    /* orange */
.flash-toned       { background: #2563eb; color: white; }    /* blue */
.flash-nsp         { background: #16a34a; color: white; }    /* green */
.flash-homegenized { background: #86efac; color: #065f46; }  /* light green */
.flash-desi        { background: #7c3aed; color: white; }    /* purple */
.flash-curd        { background: #fde047; color: #78350f; }  /* yellow */
...
.topbar .title{flex:1;text-align:center;font-weight:900;font-size:48px;color:#111827;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}

@media (max-width:760px){
    .topbar .logo{max-height:44px}
...
<header class="topbar">
    <img src="/logo/bamullogo.jpg" alt="Bamul" class="logo left">
    <div id="flash"></div>
    <div class="title">Bamul Live Crate Counter - Conveyor {{ conveyor_number }}</div>
    <img src="/logo/kmfnandini.jpg" alt="Kmfnandhini" class="logo right">
</header>
